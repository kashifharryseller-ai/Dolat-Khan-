...
export default function App() {
  ...
  return (
    <ErrorBoundary>
      ...
      <nav className="fixed top-0 w-full z-[1000] bg-midnight/80 backdrop-blur-2xl border-b border-gold/10">
        <div className="max-w-7xl mx-auto px-6 sm:px-10 py-5 sm:py-6 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-8 lg:gap-10">
            <a href="#home" className="text-sm font-medium text-white/70 hover:text-gold transition-colors relative group">Home</a>
            ...
            <a href="#subscriptions" className="bg-gradient-to-br from-accent to-accent-light text-white px-6 lg:px-8 py-2.5 lg:py-3 rounded-full text-sm font-bold shadow-lg shadow-accent/20 hover:scale-105 transition-transform">
              Subscribe
            </a>
          </div>
        ...
        <div className="max-w-5xl mx-auto px-6 text-center relative z-10">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6">
              <a href="#books" className="btn-primary w-full sm:w-auto">
                Explore Collection
              </a>
              <a href="#audiobooks" className="btn-outline w-full sm:w-auto flex items-center justify-center gap-3">
      ...
      {/* Case Laws Section */}
      <section id="caselaws" className="py-24 sm:py-32 max-w-7xl mx-auto px-6 scroll-mt-20">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <Scale className="text-gold w-6 h-6" />
              <span className="text-gold text-xs sm:text-sm font-bold tracking-[4px] uppercase">Legal Excellence</span>
            </div>
            <h2 className="font-serif text-3xl sm:text-5xl text-white">Case Laws & Constitution</h2>
            <p className="text-gold/60 max-w-2xl text-sm sm:text-base">In-depth analysis and landmark judgments curated by Dolat Khan Kakar</p>
          </div>
          <a href="#books" className="text-gold hover:text-white transition-colors flex items-center gap-2 font-bold text-sm uppercase tracking-widest">
            View All Legal Books <ArrowRight className="w-4 h-4" />
          </a>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {books.filter(b => b.category.includes('Case Laws')).map((book, i) => (
            <motion.div
              key={book.id}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              transition={{ delay: i * 0.1 }}
              viewport={{ once: true }}
              className="bg-slate p-8 rounded-[2.5rem] border border-gold/10 flex gap-8 hover:border-gold/40 transition-all group"
            >
              <div className="w-32 h-44 bg-midnight rounded-2xl flex items-center justify-center text-5xl shadow-2xl group-hover:scale-105 transition-transform overflow-hidden">
                {book.cover_icon.startsWith('http') ? (
              ...
              <div className="flex-1 space-y-4">
                <div className="space-y-1">
                  <h3 className="font-serif text-2xl text-white group-hover:text-gold transition-colors">{book.title}</h3>
                  <p className="text-white/40 text-sm italic">by {book.author}</p>
                </div>
                <p className="text-gold/60 text-sm line-clamp-2">{book.description}</p>
                <div className="flex items-center gap-4 pt-2">
                  <button 
                    onClick={() => setSelectedBook(book)}
                    className="bg-gold/10 text-gold px-6 py-2 rounded-xl text-xs font-bold hover:bg-gold hover:text-midnight transition-all border border-gold/20"
                  >
                    Read Preview
                  </button>
                  <span className="text-white font-bold">Rs. {book.price}</span>
                </div>
              </div>
            </motion.div>
          ))}
              ...
              >
                ...
                <div className="flex-1 text-center md:text-left space-y-4 relative z-10">
                  ...
                  <div className="flex items-center justify-center md:justify-start gap-6 pt-4">
                    <button 
                      onClick={() => setPlayingAudiobook(book)}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              ...
              <div className="relative">
                <div className="aspect-square bg-midnight rounded-[3rem] border border-gold/20 p-8 flex flex-col justify-center items-center text-center space-y-8 shadow-2xl">
                  ...
                  <a href="#subscriptions" className="btn-primary w-full">View All Plans</a>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-16 lg:gap-24 mb-24 sm:mb-32">
            ...
            <div className="space-y-6 sm:space-y-8">
              ...
              <ul className="space-y-3 sm:space-y-4">
                {['Home', 'Books', 'Audiobooks', 'Events', 'About'].map(link => (
                  <li key={link}>
                    <a href={`#${link.toLowerCase()}`} className="text-white/40 hover:text-gold transition-colors text-sm sm:text-base">{link}</a>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {selectedBook && (
          <div key="book-modal" className="fixed inset-0 z-[5000] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
                ...
                <div className="p-8 sm:p-12 lg:p-16 flex flex-col h-full">
                  <div className="space-y-8 flex-1">
                    <div className="space-y-4">
                      ...
                      <h2 className="font-serif text-4xl sm:text-5xl lg:text-6xl text-white leading-tight">{selectedBook.title}</h2>
                      <div className="flex items-center gap-4">
                        ...
                        <p className="text-xl text-gold/80 font-medium">by {selectedBook.author}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-6 py-4 border-y border-white/5">
                      ...
                      <span className="text-white/40 text-xs sm:text-sm font-medium tracking-wide">120+ Verified Reviews</span>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 pt-12 mt-auto">
                    <div className="bg-white/5 p-6 sm:p-8 rounded-2xl sm:rounded-3xl border border-white/10 space-y-3 sm:space-y-4 hover:border-gold transition-colors group">
                      <span className="text-gold text-[10px] sm:text-xs font-bold uppercase tracking-widest">Digital Edition</span>
                      <div className="flex items-baseline gap-2">
                        <span className="text-3xl sm:text-4xl font-bold text-white">Rs. {selectedBook.price}</span>
                      </div>
                      <button className="w-full bg-gold text-midnight py-3 sm:py-4 rounded-xl font-bold hover:bg-gold-bright transition-colors text-sm sm:text-base">
                        Get E-Book
                      </button>
                    </div>
                    <div className="bg-white/5 p-6 sm:p-8 rounded-2xl sm:rounded-3xl border border-white/10 space-y-3 sm:space-y-4 hover:border-accent transition-colors group">
                      ...
                      <button className="w-full bg-accent text-white py-3 sm:py-4 rounded-xl font-bold hover:bg-accent-light transition-colors text-sm sm:text-base">
                        Order Print
                      </button>
                    </div>
                  </div>
